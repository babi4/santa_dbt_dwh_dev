    ...
    post_hook = ["
                    ...
                    alter table {{ final_table_relation }} delete where _airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', quote(this.schema) + '.' + quote('dbgames')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', quote(this.schema) + '.' + quote('dbgames')) }}
                    ...
                    {% endif %}
                    ","drop view _airbyte_dwh_dev.dbgames_stg"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('dbgames_stg')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('dbgames_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('dbgames_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('dbgames_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
